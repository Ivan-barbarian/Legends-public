this.legend_qiyan_dances_encounter <- this.inherit("scripts/encounters/encounter", {
	...
	function isValid(_camp) {
		...
		if (roster.len() < 3)
			return false;
		local candidatesQiyan = roster.filter(@(_, _bro) _bro.getBackground().getID() == ::Legends.Backgrounds.IDs.LegendQiyan || _bro.getBackground().getID() == ::Legends.Backgrounds.IDs.BellyDancer);
