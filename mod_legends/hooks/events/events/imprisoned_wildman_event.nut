::mods_hookExactClass("events/events/imprisoned_wildman_event", function(o) {
	...
	o.create = function() {
		...
		foreach (s in this.m.Screens) {
			...
			if (s.ID == "C") {
				s.Text = "[img]gfx/ui/events/event_100.png[/img]%taskedbro% is the one tasked with cleaning out the wildling\'s corpse-featured terrarium. %They_taskedbro% rolls up %their_taskedbro% sleeves and approaches the cage with both hands out.%SPEECH_ON%Whoa there, easy now. Easy!%SPEECH_OFF%The wildling gets up off the dead body and goes to the other side of its habitat. The sellsword easily grabs the boot of the corpse and drags it toward the bars. It slips through with sickening ease, already clumped up in the fashion of discarded wet clothes. Guts and limbs dribble off the edge of the wagon\'s platform. The animal tamer cheers happily.%SPEECH_ON%Thank you so much! And you made it look so easy, too!%SPEECH_OFF%%taskedbro% stares at the dead body with the realization that could have easily been %their_taskedbro% end.%SPEECH_ON%Yeah. You\'re welcome.%SPEECH_OFF%"
			}
			if (s.ID == "D") {
