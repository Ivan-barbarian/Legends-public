::mods_hookExactClass("events/events/rookie_gets_hurt_event", function(o) {
	...
	o.onUpdateScore = function () {
		...
		foreach( bro in brothers )
			if (bro.getLevel() == 1 && bro.getBackground().getID() != ::Legends.Backgrounds.getID(::Legends.Background.Slave) && !bro.getBackground().getID() == ::Legends.Backgrounds.getID(::Legends.Background.Slave) && !bro.getBackground().isBackgroundType(this.Const.BackgroundType.Combat) && bro.getPlaceInFormation() <= 17 && bro.getLifetimeStats().Battles >= 1)
				candidates.push(bro);
