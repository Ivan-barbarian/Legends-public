::mods_hookExactClass("events/events/wound_heals_event", function(o) {
		...
		foreach (s in this.m.Screens) {
			if (s.ID == "A") {
				s.start <- function ( _event ) {
					...
					if (bg == ::Legends.Backgrounds.IDs.Monk || bg == ::Legends.Backgrounds.IDs.Flagellant || bg == ::Legends.Backgrounds.IDs.PacifiedFlagellant || bg == ::Legends.Backgrounds.IDs.MonkTurnedFlagellant || bg == ::Legends.Backgrounds.IDs.Cultist)
	...
	o.onUpdateScore = function () {
		...
		foreach( bro in brothers )
			if (bro.getBackground().getID() != ::Legends.Backgrounds.IDs.Slave && bro.getBackground().getID() != ::Legends.Backgrounds.IDs.LegendDonkey && bro.getSkills().hasSkillOfType(this.Const.SkillType.TemporaryInjury))
				candidates.push(bro);
